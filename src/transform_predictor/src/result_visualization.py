...
class ResultVisualization:
    ...
    def visualize_results(self, dataset, rgb_images, poses, predictions, te, re):
        ...
        for vis_idx in tqdm(range(len(dataset.flow_image_names))):
            ...
            axs[1, 1].imshow(dataset.flow_images[vis_idx].squeeze(0).permute(1, 2, 0).numpy().astype(np.uint8))
            axs[1, 1].set_title("Flow image")
            fig.savefig(f"{self.save_dir}/{vis_idx:05d}.jpg")
            plt.close(fig)
